...
In its current state the API is intended as a developer preview. It supports a limited amount of use cases and 8SELECT products.

## Sending requests

{% api-method method="post" host="https://api-demo.8select.io" path="/graphql" %}
{% api-method-summary %}
/graphql
{% endapi-method-summary %}

{% api-method-description %}
The GraphQL API developer preview endpoint.
...
{% api-method-parameter name="x-api-id" type="string" required=true %}
Your 8SELECT API ID.
{% endapi-method-parameter %}
...
Example response for a query to fetch product sets for a given SKU.
...
Fetch product sets for a product by given SKU.

{% tabs %}
{% tab title="GraphQL Query" %}
```graphql
query {
  productSets(input: {queryType: SKU, value: "486890-0001-09900"}) {
    edges {
      node {
        id
        title
...
{% tab title="GraphQL schema" %}
```graphql
type Query {
  productSets(input: ProductSetQueryInput): ProductSetConnection
}
